...
public class SingleGameActivity extends AppCompatActivity {
    ...
    public class Panel extends View {    //棋盘
        ...
        public void init(Canvas canvas) {
         ...
         //   canvas.drawText("打印棋盘", arm*32,arm*12, mPaint);  //棋盘
            if (judge.judgesf()!=0){
                if (judge.judgesf()==1) {
                    judge.winner= 1; r_Paint.setTextSize(60);
                    canvas.drawText("红方胜利", arm * 32, arm * 15, r_Paint);
                }
                else {
                    judge.winner=2;b_Paint.setTextSize(60);
                    canvas.drawText("蓝方胜利", arm*32,arm*15, b_Paint);
                }
            }
            else {
                if (judge.stepTurn == 0)
                        canvas.drawText("请红色方落子", arm * 32, arm * 15, mPaint);
                else  canvas.drawText("请蓝色方落子", arm * 32, arm * 15, mPaint);
            }
...
//                else if ((px>=arm*32)&&(py>=arm*10.6&&py<=arm*11.6)){   //打印棋谱
//                   String s="";
...
//                }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(new SingleGameActivity.Panel(getApplicationContext()));
        ActivityManager exitM = ActivityManager.getInstance();
        exitM.addActivity(SingleGameActivity.this);
    }
    ...
    public void onBackPressed() {
        super.onBackPressed();
        for (int i=1;i<=150;i++) {
            judge.tot=0;judge.used[i]=0; judge.winner=0;
        }
        Intent intent=new Intent(getApplicationContext(),HexActivity.class);
        startActivity(intent);
    }
